...
public class SampleTests extends ActivityInstrumentationTestCase2<CameraActivity> {
    ...
    public void testCalPixls() {
        Size[] sizes = new Size[2];
        for (int i =0; i<2; ++i) {
            sizes[i] = new Size(640, 480);
        }
        double actual = com.example.android.camera2basic.CameraActivity.calPixls(sizes);
        // expected value is 0.3
        double expected = 0.3;
        // use this method because float is not precise
        assertEquals("calculate megapixls failed", expected,
                actual, 0.001);
    }
}
